...
\section{Evaluation Using Fallout}
We evaluated our algorithm using artificial time series created using fallout. The dataset aims to mimic real world scenarios by adding noise to the time series.

We are using fallout(add citation), an internally developed tool to generate time series for performance testing of hunter. The time series are created by running a workload against NoSQL benchmark. Change points are created by injecting latency's in the workload using chaos-mesh. By changing the time and the value of injected latency's we are able to replicate different real world scenarios as well as create a ground truth datset to compare Hunter's performance against.

In order to create different time series, we decide on a group of variable that we varied to generate varying scenarios. We created 9 different scenarios by altering the values of the following variables: number of changepoints, magnitude of change of variance between groups, magnitude of change between groups and the length of groups. A group is defined as the set of points occurring between two changepoints. Each scenario contains 5 test series, each with a minor variation. The scenarios themselves can be grouped into three categories. The first scenario, change in mean, creates change points by changing the mean of the groups. The variance remains relative constant. Similarly, change in variance has constant mean and varying variance. This case is great to replicate noisy environment's. Change in both mean and variance helps us noisy replicate environment's with random latencies. 

\subsection{Extracting the ground truth}
In order to correctly measure hunters performance, we needed to have a set of true changepoints to benchmark against the predicted changepoints. Using fallout posed a slight problem because we did not have that data directly available. Even though we could estimate when we were injected chaos we still needed to extract the precise time stamp. In order to extract the timestamps we turned to the fallout logs and determined that the phase \textit{injected} marks the start of the chaos duration and the phase \textit{not injected} marks the end of the the chaos duration. As all our groups are created by injecting chaos into the system, it is reasonable to say that the timestamps associated with these phases mark our change points. 

\subsection{Evaluation Metrics}
Hunter will be evaluated using two metrics, F1 score and Rand Index. In particular we will be evaluating the \textbf{p99} metric.p99 indicates that 1 in every 100 users will encounter latency.It is considered an industry standard and thus is appropriate to qualify how well does our system work.
...
It was important to ensure that there were no duplication's ie. if two points in the predicted set were in the margin of error of the same point in $X^{*}$. A  changepoint in $X^{*}$ was marked as \textit{visited} once a point in $X$ was within $M$ and added to $TP(X,X^{*})$ set and can not be considered again. 
...
Precision is the proportion of predicted change points that are true change points:($https://arxiv.org/pdf/1801.00718.pdf$)
$$Precision  = \frac{|TP(X,X^*)|}{|X|^*}$$ 

Recall is the proportion of true change points that are well predicted:($https://arxiv.org/pdf/1801.00718.pdf$)
...
We ran hunter over 45 test runs. The test runs were ran on a GKE cluster using Kubernetes. The cluster is ran on 4 nodes in zone \textit{us-central1-a} region \textit{us-central1}. 

We will be evaluating hunter using F1 score and RandIndex. We will also be benchmarking the performance against other well-known offline algorithms PELT and DYNP. These algorithms were used using the ruptures package in python.\textbf{cite(Charles Truong; Laurent Oudre; Nicolas Vayatis;(n.d.). Selective review of offline change point detection methods)}

\subsection{Results}

All the algorithms were evaluated on two metrics. It can be seen that hunter has consistently outperformed both \textit{pelt} and \textit{randindex} on both the metrics. In all the experiments we had an margin of error as \textbf{10 seconds}.

\begin{figure}[b]
	\includegraphics[width=0.45\textwidth]{bar-graph.png}
	\caption{Correlation between F1 and number of points }\label{Figure 1}
\end{figure}

\subsubsection{Correlation to the number of points}

There is a positive correlation between the number of points and accuracy. As the number of points increase so does hunter's performance. For the Figure2 it can be seen that hunter outperformed both PELT and DYNP with a huge margine.

\subsubsection{Correlation between delta error and algorithms}
Hunter performance increases with increase in margin error. Hunter is able to get an \textit{F1 score} of \textbf{0.1481} with an delta error of one second. The performance increases drastically as we give the algorithm slightly more flexibility in terms of margin. With a margin of error of 4 seconds we see that the performance increases to \textit{0.612}
...
Scenario2    & 0.466667 & 0.305556 & 0.027027 & 0.013698 & 0.285714 & 0.166667\\
    \addlinespace%
Scenario 3    & 0.466667 & 0.305556 & 0.040556 & 0.020698 & 0.285714 & 0.166667 \\
    \addlinespace
    \midrule
